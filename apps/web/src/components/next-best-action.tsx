...
import { getNextBestActionAction } from '@/app/ai-actions'

// On-demand Next Best Action (Phase 10 #4): button → 3–5 ranked priorities.
export function NextBestAction() {
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      const res = await getNextBestActionAction()
  ...
  return (
    <div className="bg-[#111111] rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        >
          {pending ? 'Thinking…' : items ? 'Refresh' : 'Get Priorities'}
        </button>
      </div>
      ...
      {!error && items && items.length === 0 && (
        <p className="text-[11px] text-gray-600">No priorities to surface right now.</p>
      )}
      ...
      {!items && !error && !pending && (
        <p className="text-[11px] text-gray-600">Surface the most urgent priorities from current performance.</p>
      )}
    </div>
  )
}
